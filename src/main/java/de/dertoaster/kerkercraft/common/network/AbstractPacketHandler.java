...
public abstract class AbstractPacketHandler<P extends Object> implements IMessageHandler<P> {
	...
	public final void handlePacket(P packet, IPayloadContext context) {
		context.enqueueWork(() -> {
			Player sender = null;
			Level world = null;
			if(context.listener().getConnection().getPacketListener() instanceof ServerPacketListener) {
	 ...
	 */
	protected abstract void execHandlePacket(P packet, IPayloadContext context, @Nullable Level world, @Nullable Player player);
